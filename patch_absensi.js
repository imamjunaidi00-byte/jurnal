...
const oldFn = `        function renderAbsensiTable(siswaKelas, kelas) {
            const tbody = document.getElementById('absensi-table');`;

const newFn = `        function renderAbsensiTable(siswaKelas, kelas) {
...
// Replace tbody.innerHTML with container.innerHTML
html = html.replace(
    `tbody.innerHTML = html || '<tr><td colspan="5" class="text-center py-8 text-gray-400">Belum ada siswa di kelas ini</td></tr>';`,
    `container.innerHTML = html || '<div class="text-center py-8 text-gray-400">Belum ada siswa di kelas ini</div>';`
);

// Replace the tr-based html with card-based html
const oldRow = `                html += \`
...
const newRow = `                const btnActive = (st) => {
                    const c = {hadir:'bg-green-500 text-white border-green-400', sakit:'bg-yellow-500 text-white border-yellow-400', izin:'bg-blue-500 text-white border-blue-400', alpha:'bg-red-500 text-white border-red-400'};
                    return status===st ? c[st] : 'bg-white text-gray-400 border-gray-200';
                };
                html += \`
                <div class="bg-white rounded-xl border \${!status ? 'border-amber-200' : 'border-gray-100'} p-4 shadow-sm">
                    ...
                    <div class="grid grid-cols-5 gap-2 mb-2">
                        <button onclick="setAbsensi('\${key}','hadir')" class="py-3 rounded-xl text-sm font-bold border-2 transition-all active:scale-95 \${btnActive('hadir')}">H</button>
                        <button onclick="setAbsensi('\${key}','sakit')" class="py-3 rounded-xl text-sm font-bold border-2 transition-all active:scale-95 \${btnActive('sakit')}">S</button>
                        <button onclick="setAbsensi('\${key}','izin')" class="py-3 rounded-xl text-sm font-bold border-2 transition-all active:scale-95 \${btnActive('izin')}">I</button>
                        <button onclick="setAbsensi('\${key}','alpha')" class="py-3 rounded-xl text-sm font-bold border-2 transition-all active:scale-95 \${btnActive('alpha')}">A</button>
...
if (html.includes(oldRow)) {
    ...
    console.log('Row not found - trying partial match');
    const idx = html.indexOf("html += `\n                    <tr class=\"hover:bg-gray-50");
    console.log('tr found at:', idx);
}
